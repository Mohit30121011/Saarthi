...
## AI Chatbot (Module 8) — just built, needs API keys + a real test pass
Full backend implementation of SRS Section 3.8/5.6 landed this session: `ChatController` (`/api/chat/message`, `/api/chat/history`) → `ChatService` (grounding/verification per Section 5.6 — retrieves candidate schemes via existing DAOs, computes eligibility deterministically via a new `EligibilityService.evaluate()`/`evaluateCandidates()` public API instead of duplicating that logic, calls the LLM with a hard-scoped system prompt, then strips any scheme the model mentions that wasn't actually in that turn's supplied context) → `com.saarthi.integration.GeminiClient`/`GrokClient` (Gemini primary, Grok fallback, both via plain `java.net.http.HttpClient` — no new JARs needed). New DAOs `ChatSessionDAO`/`ChatHistoryDAO` against the `chat_sessions`/`chat_history` tables that were already in `schema.sql` (marked "schema-ready; feature deferred" — now built). Frontend: `frontend/src/components/ChatWidget.jsx` replaces the static fake "Ask Saarthi" widget that was previously hardcoded into `Dashboard.jsx` (its quick-suggestion buttons used to just `navigate()` to the Explorer with a query string — no real chat existed) — it's now mounted globally in `AppShell.jsx` so it's available on every logged-in page, does real message exchange, and renders matched scheme details as inline mini-cards directly in the chat bubble (linking to Scheme Detail) per FR8.8.

**Scope decisions made this session (don't re-ask):** Gemini primary / Grok fallback (not user-selectable). Scope guardrail is the "broader civic-scheme" option — the bot may also answer general Indian civic-process questions (e.g. "how do I apply for a new Aadhaar card") using general knowledge even without a matching DB row, but must never invent facts about a *specific* scheme (name/amount/eligibility/deadline/documents) outside the verified candidate set supplied that turn. Chat is **behind auth only** in this release — `/api/chat/*` was deliberately left off `AuthFilter`'s public-path list, matching the existing guest-mode-is-a-scope-cut decision already documented below (FR8.3/FR8.6 guest support is architected for — `ChatService` methods don't hard-require a DB profile — but not wired to an unauthenticated path yet).

**Before this is actually usable:**
1. **API keys are not set.** Copy `backend/WebContent/WEB-INF/classes/chatbot.properties.example` to `chatbot.properties` in that same folder (gitignored, same pattern as `db.properties`) and fill in real `gemini.api.key` / `grok.api.key`. Until then, `ChatService` returns a friendly "assistant isn't set up yet" message instead of erroring — verified this doesn't crash Tomcat startup (`ChatbotConfig` tolerates a missing/placeholder file, unlike `DBUtil` which is fatal-on-missing by design).
2. **Never actually called a real LLM.** No keys were available this session, so `GeminiClient`/`GrokClient`'s request/response shapes are implemented against each provider's documented API as of this session, but genuinely untested end-to-end — the whole backend does compile cleanly (`javac` against the full source tree, zero errors, verified this session), and the grounding/verification logic (Sections 5.6 Steps 1-4) was reasoned through carefully, but the first real run should be watched closely for: (a) whether Gemini's `responseMimeType: application/json` actually returns the exact `{"reply", "referencedSchemeIds"}` shape the system prompt asks for, (b) whether Grok's endpoint/model name (`grok-4` — a guess, verify against your actual xAI account's available models) is still current, (c) whether the daily rate limit (`chat.daily.message.limit`, default 40) feels right.
3. **Frontend build/runtime not verified either.** `npm run build` failed this session with `ENOSPC` — the machine's **C: drive was completely full (0 bytes free)** mid-session (unrelated to this feature; flagged to the user, in progress). Once there's disk space again: `npm run build` (or `npm run dev`) to catch anything the IDE's live diagnostics didn't, then click through a real chat conversation as a logged-in user.
4. **Historical chat replay is text-only.** `GET /api/chat/history` returns prior messages without reconstructing their scheme cards (only `context_scheme_ids` — raw IDs — was persisted for audit, not full card data) — the live in-session reply from `POST /api/chat/message` does include full scheme cards, this only affects re-opening the widget after a page reload mid-conversation. Acceptable simplification for now, not a bug.
5. Manual DB check worth doing once keys are added: confirm `chat_sessions`/`chat_history` rows actually appear after a real conversation, and that `flagged_unverified_mentions` stays empty in normal use (a non-empty value means the model referenced a scheme outside its supplied context — expected occasionally, worth eyeballing early on to sanity-check the system prompt is working).
