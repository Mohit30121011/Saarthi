...
export default function CustomDropdown({
  ...
  return (
    <div className={`relative ${open ? 'z-30' : ''} ${className}`} ref={dropdownRef}>
      <button
        id={id}
        type="button"
        disabled={disabled}
      ...
      >
        <div className="flex items-center gap-2 min-w-0 pr-1 flex-1">
          ...
          <span
            className={`truncate ${
              value ? (isCompact ? 'text-[#111C2D] font-bold' : 'text-[#111C2D] font-medium') : 'text-[#44474E]'
            }`}
          >
            {displayLabel}
          </span>
      ...
      {open && !disabled && (
        <div
          role="listbox"
          className={`absolute left-0 right-0 z-50 bg-white rounded-xl border border-[#CBD5E1] p-1.5 shadow-xl animate-in fade-in zoom-in-95 duration-150 ${
            openUpwards
              ? 'bottom-[calc(100%+6px)] shadow-[0_-10px_25px_-5px_rgba(13,34,64,0.18)]'
              : 'top-[calc(100%+6px)] shadow-[0_12px_28px_-6px_rgba(13,34,64,0.18)]'
          } ${menuClassName}`}
        >
          {/* Optional Search Bar if many options */}
