...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      {/* 5. FLOATING "ASK SAARTHI" AI ASSISTANT WIDGET */}
      ...
      {/* Interactive Quick Help / Ask Saarthi Modal Drawer */}
      {chatOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-white rounded-3xl shadow-2xl border border-slate-200 p-5 animate-in fade-in slide-in-from-bottom-5 duration-200">
          ...
          <div className="py-4 space-y-2">
            <p className="text-xs text-slate-600 leading-relaxed">
              Namaste {userFirstName}! What would you like assistance with today?
            </p>
            <div className="space-y-1.5 pt-1">
              {[
                'Am I eligible for PM Awas Yojana?',
                'Which documents do I need for DBT?',
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-slate-50 border border-slate-200 rounded-full px-3.5 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#156f45]"
              onKeyDown={(e) => {
                if (e.key === 'Enter' && e.target.value.trim()) {
                  setChatOpen(false)
                  navigate(`/explorer?q=${encodeURIComponent(e.target.value.trim())}`)
                }
              }}
            />
            <button
              ...
              className="w-7 h-7 rounded-full bg-[#156f45] text-white flex items-center justify-center shrink-0 shadow-xs"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
              </svg>
